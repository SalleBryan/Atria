...
Claims used: sub, the custom tenant and person attributes, and the roles the
account was created with. Roles come from the token because they are fixed at
...
def verify(token: str, *, issuer: str, audiences: tuple[str, ...]) -> Claims:
    ...
    tenant_id = payload.get(f"{CUSTOM}tenantId")
    person_id = payload.get(f"{CUSTOM}personId")
    roles = payload.get(f"{CUSTOM}roles")
